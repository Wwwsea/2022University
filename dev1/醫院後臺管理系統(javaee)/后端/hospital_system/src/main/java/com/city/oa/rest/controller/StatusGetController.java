 ...
 * 取得指定的等級对象
 */
@WebServlet("/api/work/get.do")
public class StatusGetController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(sno!=null&&sno.trim().length()>0) {
				...
				result.setStatus("N");
				result.setMessage("没有传送等級编号");
			}
		}
		catch(Exception e) {
			result.setStatus("ERROR");
			result.setMessage("取得指定等級失败 XoX....,原因:"+e.getMessage());
