 ...
 * 部門修改REST API
 */
@WebServlet("/api/area/modify.do")
public class DeptModifyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//REST发送JSON给前端
			result.setStatus("Y");
			result.setMessage("修改部門成功⎛⎝≥⏝⏝≤⎛⎝");
			
		}
		catch(Exception e) {
			result.setStatus("N");
			result.setMessage("修改部門失败 XoX....,失败 XoX....原因:"+e.getLocalizedMessage());
		}
		//将Result对象转换为JSON
