 ...
 * 等級删除REST API
 */
@WebServlet("/api/work/delete.do")
public class StatusDeleteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//REST发送JSON给前端
			result.setStatus("Y");
			result.setMessage("删除等級成功⎛⎝≥⏝⏝≤⎛⎝");
			
		}
		catch(Exception e) {
			result.setStatus("N");
			result.setMessage("删除等級失败 XoX....,失败 XoX....原因:"+e.getLocalizedMessage());
		}
		//将Result对象转换为JSON
